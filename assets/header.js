      ...
      if (window.Shopify && window.location.pathname === Shopify.routes.root) {
        ...
        let retry = setInterval(() => {
          ...
          if (window.scrollY === 0) {
            ...
            if (
              logo_check &&
              parseFloat(getComputedStyle(logo_check).paddingTop) > 0
            ) {
              clearInterval(retry); // stop immediately once passed

              setTimeout(() => {
                let headerConObj = document.querySelector(
                  "#shopify-section-header",
                );
                let global_content_tm = document.querySelector(
    ...
    methods: {
      listenMainMobileMenuClick() {
        let menus = document.querySelectorAll(
          "#header-con .header-con-menu-col-main .accordion-title",
        );
        menus.forEach((menu) => {
          menu.addEventListener("click", function () {
            let activeMenuBox = document.querySelector(
              "#header-con .header-con-menu-col-main",
            );
            let activeMenu = document.querySelector(
              "#header-con .header-con-menu-col-main .accordion-title.active",
            );
            if (activeMenu) {
      ...
      removeAnnBar() {
        ...
        if (site_header != null) {
          ...
          $360.headerHeight =
            site_header.querySelector(".header-con-outer").offsetHeight;
      ...
      getRecentKeywordsCookie() {
        const cookie = this.$360
          ? $360.getCookie("recent_keywords")
          : document.cookie
              .split("; ")
      ...
      loadRecentKeywords() {
        ...
        let recentKeywordsElem = document.querySelector(
          "#header-con__search-recent",
        );
      ...
      clickViewAll() {
        ...
        window.location.href = "/search?type=product&q=" + this.q + "";
      },
      renderSearchResults(resultsMarkup) {
        this.predictiveSearchResults.innerHTML = resultsMarkup;
        document
          .querySelector("#header-con__search-suggestion-box")
          .classList.add("hide-m");
        document
          .querySelector("#header-con__predictive-search")
          .classList.remove("hide-m");
        document
          .querySelector("#header-con__search-result")
          .classList.remove("hide-m");
        document
          .querySelector("#header-con__search-result")
          .classList.add("push-d-3");
        document
          .querySelector("#header-con__search-result")
          .classList.add("col-d-6");
        document
          .querySelector("#header-con__search-result")
          .classList.remove("col-d-12");
        document
          .querySelector("#header-con__search-result")
          .classList.remove("product-results-only");
      },
      ...
      noSearchResults() {
        ...
        document
          .querySelector("#header-con__search-result")
          .classList.remove("push-d-3");
        document
          .querySelector("#header-con__search-result")
          .classList.remove("col-d-6");
        document
          .querySelector("#header-con__search-result")
          .classList.add("col-d-12");
        document
          .querySelector("#header-con__search-result")
          .classList.add("product-results-only");
      },
      async goSearch() {
        let this_obj = this;
        setTimeout(async () => {
          if (this_obj.q.trim() == "" || this_obj.q.trim() == null) {
            ...
            fetch(
              `${routes.predictive_search_url}?q=${encodeURIComponent(this_obj.q)}&section_id=predictive-search&resources[type]=product,collection,article,page`,
            )
              .then((response) => {
                if (!response.ok) {
          ...
          try {
            let resultContainer = document.querySelector(
              "#header-con__search-result",
            );
            const suggestion_elem = document.querySelector(
              "#header-con__search-suggestion-box",
            );
            ...
            if (this_obj.q.trim() == "") {
              ...
              if (data.includes("no-result-marker")) {
                document
                  .querySelector("#header-con__dropdown-search")
                  .classList.add("no-result");
              } else {
                document
                  .querySelector("#header-con__dropdown-search")
      ...
      siteHeaderHideShowBehaviour() {
        ...
        window.addEventListener("scroll", () => {
          ...
          const scrollDirection =
            scrollPos > this_obj.prevScrollPos ? "down" : "up";
          if (
            window.Shopify &&
            window.location.pathname === Shopify.routes.root
          ) {
      ...
      toggleSubmenu(target) {
        ...
        let active_submenus = document.querySelectorAll(
          `.header-con-submenu[data-parent="${target}"]`,
        );
        let active_menu = document.querySelector(
          `.header-con-menu-item[data-parent="${target}"]`,
        );
      ...
      toggleHeaderMenu(target = null) {
        ...
        let dropdown_menu = document.querySelector(
          "#header-con__dropdown-menu",
        );
        let dropdown_search = document.querySelector(
          "#header-con__dropdown-search",
        );
        let site_header = document.querySelector("#site-header");
        if (screen.width < 1024) {
          site_header.classList.add("scroll-top");
          let announcementBar = document.querySelector("#header-bar");
          const headerConObj = document.querySelector(
            "#shopify-section-header",
          );
          let hideHeight = 0;
          if (announcementBar != null) {
      ...
      toggleGiftWrapDetails() {
        ...
        let anyChecked = document.querySelector(
          ".gift-wrap-main-product:checked",
        );
      ...
      selectGiftWrapDesign(event) {
        ...
        let gwp_vars = document.querySelectorAll(
          ".header-con__gift-wrap-products .gwp-var",
        );
      ...
      countGiftWrapPrice() {
        let gift_wrap_product = document.querySelector(
          ".gift-wrap-product-data",
        );
        let gwp_price = gift_wrap_product.getAttribute("data-price");
        let gift_wrap_btn = document.querySelector(".btn-add-gift-wrap");
        let gift_wrap_btn_price = gift_wrap_btn.querySelector(
          ".gift-wrap-total-price",
        );
        let gift_wrap_btn_label = gift_wrap_btn.querySelector(
          ".gift-wrap-total-label",
        );
        let gift_wrap_products = document.querySelectorAll(
          ".hc__gw-item.active",
        );
        let total_price = 0;
        gift_wrap_products.forEach((item) => {
          ...
          let gift_wrap_price = item
            .querySelector(".gwp-var")
            ?.getAttribute("data-price");
          let selected_var = item.querySelector(".gwp-var.selected")?.value;
          let selected_price = item
            .querySelector(".gwp-var.selected")
            ?.getAttribute("data-price");
          if (selected_var) {
      ...
      async addGiftWrapProduct(items = [], to_delete = []) {
        ...
        try {
          ...
          if (to_delete.length > 0) {
            const deleteOld = await axios.post(
              "/cart/update.js",
              deletePayload,
            );
          }
      ...
      async processGiftWrapProduct(e) {
        ...
        if (!target.classList.contains("disabled")) {
          ...
          gift_wrap_products.forEach((item) => {
            ...
            let orig_gift_wrap_variant =
              orig_gift_wrap?.getAttribute("data-variant");
            let orig_gift_wrap_sender =
              orig_gift_wrap?.getAttribute("data-sender");
            let orig_gift_wrap_recipient =
              orig_gift_wrap?.getAttribute("data-recipient");
            let orig_gift_wrap_message =
              orig_gift_wrap?.getAttribute("data-message");
            let parent_qty = item
              .querySelector(".gift-wrap-main-product")
            ...
            let gift_wrap_var = item.querySelector(".gwp-var")?.value;
            let selected_var = item.querySelector(".gwp-var.selected")?.value;
            let sender_name = document.querySelector(
              ".hc__gwi-details .gift-wrap-sender-name",
            )?.value;
            let recipient_name = document.querySelector(
              ".hc__gwi-details .gift-wrap-recipient-name",
            )?.value;
            let message = document.querySelector(
              ".hc__gwi-details .gift-wrap-message",
            )?.value;
            let leave_blank = document.querySelector(
              ".hc__gwi-details .gift-leave-blank",
            );
            if (selected_var) {
              gift_wrap_var = selected_var;
            }
            if (
              orig_gift_wrap_variant != gift_wrap_var ||
              orig_gift_wrap_sender != sender_name ||
              orig_gift_wrap_recipient != recipient_name ||
              orig_gift_wrap_message != message
            ) {
              if (orig_gift_wrap) {
      ...
      uncheckLeaveblank(e) {
        ...
        let recipient_name = target.querySelector(
          ".gift-wrap-recipient-name",
        )?.value;
      ...
      toggleEmptyProduct(e) {
        ...
        let links = document.querySelectorAll(
          ".header-con__empty-links .hcel-link",
        );
        let products = document.querySelectorAll(
          ".header-con__empty-product .hcep-product",
        );
        let product = document.querySelector(
          `.header-con__empty-product .hcep-product[data-index="${idx}"]`,
        );
      ...
      chooseDelivery(el) {
        ...
        let selected_option = parent
          .querySelector('input[name="shipping-option"]:checked')
          ?.getAttribute("value");
        let pickup_option = parent.querySelector(".pickup-options");
        parent
          .querySelector(".delivery-option-item.active")
        ...
        opt.forEach((el) => {
          const name = el
            .querySelector(".checkoutMethodName")
            ?.innerText.trim();
          if (
            (method === "Shipping" && name === "Local Delivery") ||
            (method !== "Shipping" && name === "Local Delivery")
          ) {
            el.click();
            target.classList.add("active");
      ...
      selectShippingOption(el) {
        ...
        let date = document.querySelector(
          "#header-con #storePickupApp .Zapiet-InputWithPrefix__Input",
        )?.value;
      ...
      async goCheckout(el) {
        if (!el.classList.contains("disabled")) {
          try {
            ...
            let delivery_option = document
              .querySelector(".delivery-option-item.active")
              ?.getAttribute("data-method");
            let shipping_option = document
              .querySelector(
                '.header-con__delivery-options .shipping-options .shipping-option input[name="shipping-option"]:checked',
              )
              ?.getAttribute("value");
...
const wireZapietEvents = () => {
  let checkout_btn = document.querySelector(
    "#header-con #header-cart .button-checkout",
  );
  try {
